...
TensorIntrin dot_1x4x16_int8_int8_int32_avx2() {
  ...
  auto C = tvm::compute(
    {16},
    [&](Var i) {
      return tvm::sum(tvm::cast(Int(32), data(k)) *
                      tvm::cast(Int(32), kernel(i, k)), {k});
    },
    "tensor",
    "dense"
    );
  ...
  Expr llvm_pair = make_const(UInt(32), llvm::Function::lookupIntrinsicID(pair));
  Expr llvm_quad = make_const(UInt(32), llvm::Function::lookupIntrinsicID(quad));
  ...
  Stmt reduce_update_0 = c_buf.vstore({0}, quad_reduction_0 + c_buf.vload({0}, Int(32, 8)));
  Stmt reduce_update_1 = c_buf.vstore({8}, quad_reduction_1 + c_buf.vload({8}, Int(32, 8)));
...
TensorIntrin dot_16x1x16_int8_int8_int32_avx512() {
  ...
  Stmt reduce_update = c_buf.vstore({0}, quad_reduction + c_buf.vload({0}, Int(32, 16)));
...
inline Schedule schedule_quantized_mm_dequantize(
  ...
  auto _schedule_quantized_mm = [&](const Tensor& input) {
    ...
    if (*as_const_int(x_dim_size) >= 16) {
      ...
      s[input].unroll(y);
      if (target->options_array[0].as<tvm::ir::StringImm>()->value == "-mcpu=skylake-avx512") {
